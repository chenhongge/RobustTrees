...
struct TrainParam : public dmlc::Parameter<TrainParam> {
  ...
  DMLC_DECLARE_PARAMETER(TrainParam) {
    ...
    DMLC_DECLARE_FIELD(splitsample_bynode)
        .set_range(0.0f, 1.0f)
        .set_default(1.0f)
        .describe("Subsample ratio for updating split, simulate extremly randomized trees (ExtraTree)"); 
...
struct XGBOOST_ALIGNAS(16) GradStats {
  ...
  inline void Subtract(GradientPair p) { this->Subtract(p.GetGrad(), p.GetHess()); }
  /*!
  ...
  inline void Subtract(const std::vector<GradientPair>& gpair, const MetaInfo& info,
                  bst_uint ridx) {
    const GradientPair& b = gpair[ridx];
    this->Subtract(b.GetGrad(), b.GetHess());
  }
...
template <typename ParamT>
  ...
  inline void Subtract(const GradStats& b) {
    sum_grad -= b.sum_grad;
    sum_hess -= b.sum_hess;
  }
  ...
  /*! \brief set current value to a + b */
  inline void SetUnion(const GradStats& a, const GradStats& b) {
    sum_grad = a.sum_grad + b.sum_grad;
    sum_hess = a.sum_hess + b.sum_hess;
  }
  /*! \brief set current value to a */
  inline void SetCopy(const GradStats& a) {
    sum_grad = a.sum_grad;
    sum_hess = a.sum_hess;
  }
  ...
  inline void Subtract(double grad, double hess) {
    sum_grad -= grad;
    sum_hess -= hess;
  }
};
...
struct SplitEntry {
  ...
  inline void update_split_value(bst_float new_split_value) {
